...
def render_plotly_restaurant_layout_no_cache(_restaurant, path=None, title="餐厅布局"):
    """
    なし缓存版本的餐厅布局渲染函数，确保每次都重新渲染最新的布局。
...
def render_rag_test():
    ...
    # 检查RAG模块是否准备就绪
    if not rag.is_ready():
        st.warning("未加载知识库或API密钥未设置，将使用纯LLM回答或なし法获得回应")
    else:
    ...
    with test_tabs[0]:
        st.subheader("直接问答测试")
        query = st.text_input("请输入问题:（例：现在あり订单3号桌，5号桌，8号桌，告诉我配送顺序）", key="qa_query")
    ...
    with test_tabs[1]:
        st.subheader("思考层测试")
        query = st.text_input("请输入问题:（例：现在あり订单3号桌，5号桌，8号桌，告诉我配送顺序）", key="thinking_query")
